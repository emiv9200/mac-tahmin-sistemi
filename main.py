...
def send_telegram_message(text: str, parse_mode="Markdown", disable_preview=True):
    ...
    payload = {"chat_id": TELEGRAM_CHAT_ID, "text": text, "parse_mode": parse_mode, "disable_web_page_preview": disable_preview}
    try:
...
def format_prediction_message(match):
    risk_emoji = {'LOW': '🟢','MEDIUM': '🟡','HIGH': '🔴'}.get(match.get('risk_level', 'HIGH'), '⚪')
...
"""
    ...
    for match in filtered_matches:
        ...
        log_telegram_message(match['match_id'], message, success, None if success else "Send failed")
        if success:
...
def start_scheduler():
    ...
    scheduler.add_job(lambda: collect_today_matches(TARGET_LEAGUES),
                      CronTrigger(hour=7, minute=0, timezone="UTC"),
                      id="collect_morning", replace_existing=True, max_instances=1)
    scheduler.add_job(analyze_and_update_predictions,
                      CronTrigger(minute=5, timezone="UTC"),
                      id="analyze_hourly", replace_existing=True, max_instances=1)
    scheduler.add_job(lambda: send_daily_predictions(min_confidence=70, max_risk='LOW'),
                      CronTrigger(minute=10, hour="*/2", timezone="UTC"),
                      id="send_predictions", replace_existing=True, max_instances=1)
    scheduler.add_job(check_and_update_results,
                      CronTrigger(hour=23, minute=30, timezone="UTC"),
                      id="check_results", replace_existing=True, max_instances=1)
...
def setup_endpoint():
    try:
        ...
        return jsonify({"success": False, "error": str(e), "message": "Failed to create tables. Check logs for details."}), 500
...
def run_endpoint():
    try:
        ...
        sent = send_daily_predictions()
        return jsonify({"success": True, "collected": collected, "analyzed": analyzed, "sent": sent})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.getenv("PORT", 5000)))**requirements.txt**
# Web Framework
Flask==3.0.3
...
plotly==5.18.0
pandas==2.1.4
apscheduler==3.10.4**runtime.txt**
python-3.11.9
